        ...
        <form class="flex items-center max-w-lg mx-auto" method="get" action="/tasks">
            ...
            <div class="relative w-full">
                ...
                <select name="selectByTag" id="voice-search" onchange="this.form.submit()"
                    ...
                    <%
                        Long selectedTagId = (Long) request.getAttribute("selectedTagId");
                        List<Tag> tagsForSelect = (List<Tag>) request.getAttribute("tags");
                        for (Tag tag : tagsForSelect) {
                            boolean isSelected = (selectedTagId != null && selectedTagId.equals(tag.getId()));
                    %>
                    ...
                    <div class="relative p-3 rounded shadow bg-white border-t-2 <%= isPastEndDate ? "border-red-700 filtered" : isNearEndDate ? "border-yellow-200" : "border-blue-700" %>  <%= (task.getUser().getId().equals(SessionUser.getId()) || SessionUser.getUserType().name().equals("MANAGER"))? "":"filtered"%>"
                        ...
                        <div class="flex items-center mt-2">
                            <button data-modal-target="assigned-modal-<%= task.getId() %>" data-modal-toggle="assigned-modal-<%= task.getId() %>" class="block text-white" type="button">
                                <div class="flex">
                                    <img src="../assets/images/me.png" alt="Assignee" class="w-6 h-6 rounded-full bg-gray-300">
                                    <%
